  ...
  def student_actions
    ...
    if object.deleted == 'Active'
      links << h.link_to(h.content_tag(:i, nil, class: 'fas fa-trash-alt') + 'Deactive'.html_safe,
                         h.active_student_student_path(object), data: { turbo_method: :patch, turbo_confirm: 'Are you sure You Want To Deactivate?' }, remote: true, class: 'parent_user_deactive')
      links << h.link_to(h.content_tag(:i, nil, class: 'fas fa-edit') + ' Edit'.html_safe,
                         h.edit_student_path(object), class: 'parent_user_edit')                   
    else
